...
class ChatApp {
    ...
    _getApiKey() {
        this.apiKey = window.HUGGING_FACE_API_KEY;
    
        // Fallback para desarrollo local
        if (!this.apiKey || this.apiKey.startsWith('%')) {
            this.apiKey = localStorage.getItem('hf_api_key_local');
            if (!this.apiKey) {
                this.apiKey = prompt('DEVELOPMENT: Introduce tu API Key (se guardará localmente):');
                if (this.apiKey) localStorage.setItem('hf_api_key_local', this.apiKey);
            }
        }
        
        if (!this.apiKey || this.apiKey.startsWith('%')) {
             console.error("API Key no encontrada. Configúrala en Vercel y asegúrate de que el nombre de la variable coincide.");
             this.dom.chatLog.innerHTML = `<div class="text-red-400 p-4">Error de configuración: La clave de la API no está disponible.</div>`;
        }
    }
